<?php
    ...
    if ($_POST) { // se o usuario clicou no botão atualizar
        if ($_FILES['imagemfile']['name']) { // se o usuario escolher uma imagem
            unlink("../images/".$_POST['imagem_atual']); // apaga um arquivo do file server ao passar o caminho dela
            $nome_img = $_FILES['imagemfile']['name']; // recupera o nome da imagem por tag html do type file
            $tmp_img = $_FILES['imagemfile']['tmp_name']; // recupera a imagem por tag html do type file
            $rand = rand(100001,999999); // gera um numero aleatorio para a imagem e diferir a imagem de uma mesmo nome que outro usuario salvou
            $dir_img = "../images/".$rand."-".$nome_img; // caminho para salvar a imagem.
        ...
        $update = "UPDATE produtos SET tipo_id = $id_tipo,destaque = '$destaque', descricao = '$descricao', resumo = '$resumo',valor = $valor, imagem = '$nome_img' WHERE id = $id";
    ...
    $listaTipo = $conn->query("SELECT * FROM tipos ORDER BY rotulo ASC");
            ...
            <div class="col-xs-12 col-sm-offset-2 col-sm-6  col-md-8">
                ...
                <div class="thumbnail">
                    <div class="alert alert-danger" role="alert">
                        <form action="produto_atualiza.php" method="post" name="form_insere"
